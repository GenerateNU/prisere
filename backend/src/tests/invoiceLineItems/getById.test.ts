...
describe("Invoice get by id", () => {
    ...
    test("GET /quickbooks/invoice/line/:id - id that exists", async () => {
        const response = await app.request(`/quickbooks/invoice/line/${seededInvoiceLineItemId}`);
    ...
    test("GET /quickbooks/invoice/line/:id - id doesn't exist", async () => {
        const response = await app.request(`/quickbooks/invoice/line/8d720d89-d047-4f19-a999-1934f914908d`); // bad uuid
    ...
    test("GET /quickbooks/invoice/line/:id - invalid UUID", async () => {
        const response = await app.request(`/quickbooks/invoice/line/8d720d89-134f914908d`); // invalid uuid
