...
const INCIDENT_CODES = Object.keys(INCIDENT_MAPPING);
export const LABEL_TO_CODE = Object.fromEntries(Object.entries(INCIDENT_MAPPING).map(([code, label]) => [label, code]));
export const incidentTypeString = z
    .string()
    .nullable()
    .refine((s) => {
    /**
    ...
    return codes.length > 0 && codes.every((c) => INCIDENT_CODES.includes(c));
});
